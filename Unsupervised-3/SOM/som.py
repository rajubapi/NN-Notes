...
class SOM(object):
    ...
    def initialize(self, data, how='pca'):
        ...
        if how == 'pca':
            ...
            for i in range(2):
                self.map[np.random.randint(0, self.x), np.random.randint(
                    0, self.y)] = eivalues[i]
    ...
    def fit(self, data, targets, epochs=0, save_e=False, interval=1000, decay='hill'):
        ...
        if save_e:  # save the error to history every "interval" epochs
            ...
            for i in range(epochs):
                self.cycle(data[indx[i]])
                images.append(self.plot_point_map(
                    data, targets, ['Class 0', 'Class 1'], i))
    ...
    def distance_map(self, metric='euclidean'):
        ...
        for x in range(self.x):
            for y in range(self.y):
                d = cdist(self.map[x, y].reshape(
                    (1, -1)), self.map.reshape((-1, self.map.shape[-1])), metric=metric)
                dists[x, y] = np.mean(d)
    ...
    def winner_neurons(self, data):
        ...
        self.winner_indices = np.array(
            rslt, dtype='int').reshape((len(data), 2))
    ...
    def plot_point_map(self, data, targets, targetnames, epoch, filename=None, colors=None, markers=None, mol_dict=None,
        ...
        if not colors:
            colors = ['#EDB233', '#90C3EC', '#C02942',
                      '#79BD9A', '#774F38', 'gray', 'black']
        if activities:
        ...
        if not activities:
            patches = [mptchs.Patch(color=colors[i], label=targetnames[i])
        ...
        if mol_dict:
            for k, v in mol_dict.items():
                ...
                plt.annotate(k, xy=(x + 0.5, y - 0.18),
                             textcoords='data', fontsize=18, fontweight='bold')
    ...
    def plot_class_density(self, data, targets, t=1, name='actives', colormap='Oranges', mol_dict=None, filename=None):
        ...
        ax.set_aspect('equal')
        plt.text(0.1, -1., "%i Datapoints" %
                 len(t_data), fontsize=20, fontweight='bold')
    ...
    def plot_error_history(self, color='orange', filename=None):
        ...
        if not len(self.history):
            raise LookupError(
                "No error history was found! Is the SOM already trained?")
        fig, ax = plt.subplots()
        ax.plot(range(0, self.epoch, self.interval),
                self.history, '-o', c=color)
